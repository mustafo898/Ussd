...
import com.example.ussdaplication.App.Companion.resources
...
fun activate(context: Context, code: String) {
    try {
        if (!App.IS_DUAL) {
            ...
            if (getOperatorsName(context)[0].contains(App.sharedPreference.operator) || getOperatorsName(
                    context
                )[1].contains(App.sharedPreference.operator)
            ) {
                Toast.makeText(context, "Activated", Toast.LENGTH_SHORT).show()
            } else {
                Toast.makeText(
